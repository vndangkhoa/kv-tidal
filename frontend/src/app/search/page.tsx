    ...
    <div className="space-y-8 pb-16">
      ...
      <div>
        <div className="flex items-center space-x-2.5">
          <h1 className="text-2xl font-bold text-textPrimary">Search & Lossless Discovery</h1>
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="relative flex items-center">
            <input
              ...
              placeholder="Search songs, artists, albums, or paste Tidal/Qobuz URL..."
              className="w-full bg-card border border-border rounded-full px-4 py-3 pl-11 pr-32 text-sm text-textPrimary placeholder-textSecondary/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/40 transition-all shadow-lg"
            />
            <Search className="w-5 h-5 text-textSecondary absolute left-3.5 top-3" />
            ...
            {query.length > 0 && (
              <button
                type="button"
                onClick={handleClear}
                className="absolute right-24 text-textSecondary hover:text-textPrimary p-1 rounded-full hover:bg-cardHover transition-colors cursor-pointer"
                title="Clear input"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <button
              type="submit"
              disabled={loading}
              className="absolute right-2 px-4 py-1.5 bg-primary text-black font-bold rounded-full text-xs hover:brightness-110 active:scale-95 transition-all flex items-center space-x-1.5 shadow-md cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg border transition-all cursor-pointer flex items-center space-x-1 ${
            ...
            >
              ...
              <span>DR12+ Dynamic</span>
            </button>

            {/* Sort Dropdown when results are displayed */}
            {searched && results.length > 0 && (
              <div className="flex items-center space-x-1 ml-2 border-l border-border/70 pl-2">
                <ArrowUpDown className="w-3 h-3 text-textSecondary" />
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as any)}
                  aria-label="Sort search results by"
                  className="bg-card border border-border rounded-lg px-2 py-1 text-xs text-textPrimary focus:outline-none focus:border-primary cursor-pointer"
                >
                  <option value="relevance">Sort: Relevance</option>
                  <option value="quality">Sort: Hi-Res Sample Rate</option>
                  <option value="dr">Sort: DR Score</option>
        ...
        <div className="space-y-6">
          ...
          {sortedResults.length > 0 ? (
            <div className="space-y-6">
              ...
              {topMatch && (
                <div className="bg-gradient-to-r from-card via-card/90 to-surface border border-border rounded-2xl p-5 shadow-xl flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-72 h-72 bg-primary/5 rounded-full blur-3xl pointer-events-none" />
                  
                  {/* Big Artwork with Play Overlay */}
                  <div className="relative w-36 h-36 md:w-40 md:h-40 rounded-xl overflow-hidden bg-surface border border-border flex-shrink-0 shadow-lg group">
                    {topMatch.cover_url ? (
                      <img
